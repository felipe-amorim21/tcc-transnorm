...
class DecoderBlockSimple(nn.Module):
    def __init__(
        ...
        super().__init__()
        # As convoluções precisam ser ajustadas para o novo fluxo
        self.conv1 = Conv2dReLU(
            in_channels + skip_channels, # A entrada ainda é a concatenação
            out_channels,
            kernel_size=3,
            padding=1,
            use_batchnorm=use_batchnorm,
    ...
    def forward(self, x, skip=None): # Removemos o 'att' que não será usado
        x = self.up(x)
        if skip is not None:
            x = torch.cat([x, skip], dim=1)
        
        # Aplicamos as convoluções diretamente, sem os gates de atenção
        x = self.conv1(x)
        x = self.conv2(x)
        return x
...
class DecoderCupSimple(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        head_channels = 512
        ...
        # Use o nosso novo DecoderBlockSimple
        blocks = [
            DecoderBlockSimple(in_ch, out_ch, sk_ch) for in_ch, out_ch, sk_ch in zip(in_channels, out_channels, skip_channels)
        ]
        self.blocks = nn.ModuleList(blocks)

    def forward(self, hidden_states, features=None): # Removemos 'att'
        ...
        for i, decoder_block in enumerate(self.blocks):
            ...
            x = decoder_block(x, skip=skip) # Passamos apenas x e skip
        return x
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.decoder = DecoderCupSimple(config)
